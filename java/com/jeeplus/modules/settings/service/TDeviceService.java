...
//	}
	...
	public List<MapEntity> deviceList(String orgId) {
        ...
        List<MapEntity> channelList = tDeviceDao.deviceChannelListByOrgId(orgId);// 查询设备通道所有数据
        
        for (MapEntity entity : deviceList) {
			Integer devType = (Integer) entity.get("dev_type");
			String devId = (String) entity.get("id");
			String channelName = "";
			String addrName = "";
//			List<MapEntity> channelList = tDeviceDao.deviceChannelList(devId);
			for (MapEntity channel : channelList) {
                String dev_id = (String) channel.get("devId");
                if (StringUtils.isNotBlank(devId) && StringUtils.isNotBlank(dev_id) && devId.equals(dev_id)) {
